...
Assert::throws(\InvalidArgumentException::class, fn() => \Temporal\Now::zonedDateTimeISO($timeZone), 'bare date-time string is not a time zone');
...
Assert::throws(\InvalidArgumentException::class, fn() => \Temporal\Now::zonedDateTimeISO($timeZone), "ISO string {$timeZone} with a sub-minute offset is not a valid time zone");
}
$timeZone = '2021-08-19T17:30Z';
$result1 = \Temporal\Now::zonedDateTimeISO($timeZone);
Assert::sameValue($result1->timeZoneId, 'UTC', 'date-time + Z is UTC time zone');
$timeZone = '2021-08-19T17:30-07:00';
$result2 = \Temporal\Now::zonedDateTimeISO($timeZone);
Assert::sameValue($result2->timeZoneId, '-07:00', 'date-time + offset is the offset time zone');
$timeZone = '2021-08-19T17:30[UTC]';
$result3 = \Temporal\Now::zonedDateTimeISO($timeZone);
Assert::sameValue($result3->timeZoneId, 'UTC', 'date-time + IANA annotation is the IANA time zone');
$timeZone = '2021-08-19T17:30Z[UTC]';
$result4 = \Temporal\Now::zonedDateTimeISO($timeZone);
...
$result5 = \Temporal\Now::zonedDateTimeISO($timeZone);

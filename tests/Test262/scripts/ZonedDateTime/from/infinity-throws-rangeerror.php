...
Assert::throws(\InvalidArgumentException::class, function () use (&$base, &$prop, &$inf, &$overflow) { return \Temporal\Spec\ZonedDateTime::from(array_merge($base, [$prop => $inf]), ['overflow' => $overflow]); }, "{$prop} property cannot be {$inf} (overflow {$overflow}");
$calls = [];
$obj = TemporalHelpers::toPrimitiveObserver($calls, $inf, $prop);
Assert::throws(\InvalidArgumentException::class, function () use (&$base, &$prop, &$obj, &$overflow) { return \Temporal\Spec\ZonedDateTime::from(array_merge($base, [$prop => $obj]), ['overflow' => $overflow]); }, '');

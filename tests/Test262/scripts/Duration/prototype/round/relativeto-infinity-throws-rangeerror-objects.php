...
Assert::throws(\InvalidArgumentException::class, function () use (&$instance, &$base, &$prop, &$inf) { return $instance->round((object) ['smallestUnit' => 'seconds', 'relativeTo' => (object) array_merge((array) $base, [$prop => $inf])]); }, "{$prop} property cannot be {$inf} in relativeTo");
$calls = [];
$obj = TemporalHelpers::toPrimitiveObserver($calls, $inf, $prop);
Assert::throws(\InvalidArgumentException::class, function () use (&$instance, &$base, &$prop, &$obj) { return $instance->round((object) ['smallestUnit' => 'seconds', 'relativeTo' => (object) array_merge((array) $base, [$prop => $obj])]); }, '');

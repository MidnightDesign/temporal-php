...
$blankInstance->total(JsUndefined::strip(['unit' => 'minutes', 'relativeTo' => $relativeTo]));
Assert::throws(\InvalidArgumentException::class, function () use (&$instance, &$relativeTo) { return $instance->total(JsUndefined::strip(['unit' => 'minutes', 'relativeTo' => $relativeTo])); }, "\"{$relativeTo}\" is outside the representable range for a relativeTo parameter after conversion to DateTime");
}
...
Assert::throws(\InvalidArgumentException::class, function () use (&$instance, &$relativeTo) { return $instance->total(JsUndefined::strip(['unit' => 'minutes', 'relativeTo' => $relativeTo])); }, "\"{$relativeTo}\" is outside the representable range for a relativeTo parameter");
Assert::throws(\InvalidArgumentException::class, function () use (&$blankInstance, &$relativeTo) { return $blankInstance->total(JsUndefined::strip(['unit' => 'minutes', 'relativeTo' => $relativeTo])); }, "\"{$relativeTo}\" is outside the representable range for a relativeTo parameter");
}

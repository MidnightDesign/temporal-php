<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainYearMonth/from/argument-string-limits.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Temporal\Tests\Test262\Assert;
use Temporal\Tests\Test262\JsUndefined;
$validStrings = ['-271821-04', '-271821-04-01', '-271821-04-01T00:00', '+275760-09', '+275760-09-30', '+275760-09-30T23:59:59.999999999'];
foreach ($validStrings as $arg) {
\Temporal\Spec\PlainYearMonth::from($arg);
}
$invalidStrings = ['-271821-03-31', '-271821-03-31T23:59:59.999999999', '+275760-10', '+275760-10-01', '+275760-10-01T00:00'];
foreach ($invalidStrings as $arg) {
Assert::throws(\RangeException::class, function () use (&$arg) { return \Temporal\Spec\PlainYearMonth::from($arg); }, "\"{$arg}\" is outside the representable range of PlainYearMonth");
}

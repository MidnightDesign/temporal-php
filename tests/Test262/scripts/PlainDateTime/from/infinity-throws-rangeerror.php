...
Assert::throws(\InvalidArgumentException::class, function () use (&$base, &$prop, &$inf, &$overflow) { return \Temporal\Spec\PlainDateTime::from(JsUndefined::strip(array_merge($base, [$prop => $inf])), JsUndefined::strip(['overflow' => $overflow])); }, "{$prop} property cannot be {$inf} (overflow {$overflow}");
$calls = [];
$obj = TemporalHelpers::toPrimitiveObserver($calls, $inf, $prop);
Assert::throws(\InvalidArgumentException::class, function () use (&$base, &$prop, &$obj, &$overflow) { return \Temporal\Spec\PlainDateTime::from(JsUndefined::strip(array_merge($base, [$prop => $obj])), JsUndefined::strip(['overflow' => $overflow])); }, '');

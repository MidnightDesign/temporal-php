...
Assert::throws(\InvalidArgumentException::class, function () use (&$base, &$prop, &$inf, &$overflow) { return \Temporal\Spec\PlainDateTime::from((object) array_merge((array) $base, [$prop => $inf]), (object) ['overflow' => $overflow]); }, "{$prop} property cannot be {$inf} (overflow {$overflow}");
$calls = [];
$obj = TemporalHelpers::toPrimitiveObserver($calls, $inf, $prop);
Assert::throws(\InvalidArgumentException::class, function () use (&$base, &$prop, &$obj, &$overflow) { return \Temporal\Spec\PlainDateTime::from((object) array_merge((array) $base, [$prop => $obj]), (object) ['overflow' => $overflow]); }, '');

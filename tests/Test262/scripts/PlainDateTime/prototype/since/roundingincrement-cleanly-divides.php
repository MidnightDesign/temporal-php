...
Assert::assertTrue($later->since($earlier, $options) instanceof \Temporal\Duration, "valid hour increments divide into 24 (rounding increment = {$roundingIncrement}, smallest unit = hours)");
}
...
Assert::assertTrue($later->since($earlier, $options) instanceof \Temporal\Duration, "valid {$smallestUnit} increments divide into 60 (rounding increment = {$roundingIncrement})");
}
}
...
$options = ['smallestUnit' => $smallestUnit, 'roundingIncrement' => $roundingIncrement];
Assert::assertTrue($later->since($earlier, $options) instanceof \Temporal\Duration, "valid {$smallestUnit} increments divide into 1000 (rounding increment = {$roundingIncrement})");
}
}

<?php

declare(strict_types=1);

// Source: tests/Test262/data/Instant/compare/argument-string-limits.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Temporal\Tests\Test262\Assert;
use Temporal\Tests\Test262\JsUndefined;
$instance = new \Temporal\Spec\Instant(0);
$validStrings = ['-271821-04-20T00:00Z', '-271821-04-19T23:00-01:00', '-271821-04-19T00:00:00.000000001-23:59:59.999999999', '+275760-09-13T00:00Z', '+275760-09-13T01:00+01:00', '+275760-09-13T23:59:59.999999999+23:59:59.999999999'];
foreach ($validStrings as $arg) {
\Temporal\Spec\Instant::compare($arg, $instance);
\Temporal\Spec\Instant::compare($instance, $arg);
}
$invalidStrings = ['-271821-04-19T23:59:59.999999999Z', '-271821-04-19T23:00-00:59:59.999999999', '-271821-04-19T00:00:00-23:59:59.999999999', '+275760-09-13T00:00:00.000000001Z', '+275760-09-13T01:00+00:59:59.999999999', '+275760-09-14T00:00+23:59:59.999999999'];
foreach ($invalidStrings as $arg) {
Assert::throws(\RangeException::class, function () use (&$arg, &$instance) { return \Temporal\Spec\Instant::compare($arg, $instance); }, "\"{$arg}\" is outside the representable range of Instant (first argument)");
Assert::throws(\RangeException::class, function () use (&$instance, &$arg) { return \Temporal\Spec\Instant::compare($instance, $arg); }, "\"{$arg}\" is outside the representable range of Instant (second argument)");
}

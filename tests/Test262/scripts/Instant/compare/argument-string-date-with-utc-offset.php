...
Assert::throws(\RangeException::class, function () use (&$arg, &$epoch) { return \Temporal\Spec\Instant::compare($arg, $epoch); }, "\"{$arg}\" UTC offset without time is not valid for Instant (first argument)");
Assert::throws(\RangeException::class, function () use (&$epoch, &$arg) { return \Temporal\Spec\Instant::compare($epoch, $arg); }, "\"{$arg}\" UTC offset without time is not valid for Instant (second argument)");
}

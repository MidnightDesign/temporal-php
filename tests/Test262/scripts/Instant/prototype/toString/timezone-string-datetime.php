...
Assert::throws(\InvalidArgumentException::class, function () use (&$instance, &$timeZone) { return $instance->toString(['timeZone' => $timeZone]); }, 'bare date-time string is not a time zone');
...
Assert::throws(\InvalidArgumentException::class, function () use (&$instance, &$timeZone) { return $instance->toString(['timeZone' => $timeZone]); }, "ISO string {$timeZone} with a sub-minute offset is not a valid time zone");
}
